...
*/
function binomcoefln( x, y, options ) {
	/* jshint newcap:false */
	var opts = {},
		ctor,
		err,
	...
	if ( arguments.length < 2 ) {
		throw new Error( 'binomcoefln()::`y` argument is missing.' );
	}
	...
	if ( isNumber( x ) || isnan( x ) ) {
		...
		if ( isMatrixLike( y ) ) {
			...
			return options ? binomcoefln( x, y, options ) : binomcoefln( x, y );
		}
		if ( isArrayLike( y ) ) {
			return options ? binomcoefln( fill( y.length, x ), y, options ) : binomcoefln( fill( y.length, x ), y );
		}
		if ( !isNumber( y ) ) {
			return NaN;
		}
		return binomcoefln1( x, y );
	}
	if ( arguments.length > 2 ) {
	...
	if ( isTypedArrayLike( x ) ) {
		...
		return binomcoefln6( out, x, y );
	}
	if ( isArrayLike( x ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return binomcoefln4( x, y, opts.path, opts.sep );
		}
		...
		if ( opts.copy === false ) {
			...
			if ( ctor === null ) {
				throw new TypeError( 'binomcoefln()::invalid input argument. Unrecognized/unsupported array-like object. Provide either a plain or typed array. Value: `' + x + '`.' );
			}
			out = new ctor( x.length );
		}
		else {
		...
		if ( opts.accessor ) {
			return binomcoefln3( out, x, y, opts.accessor );
		}
		return binomcoefln2( out, x, y );
	}
	return NaN;
