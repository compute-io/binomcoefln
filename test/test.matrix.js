...
var // Expectation library:
	...
	// Module to be tested:
	binomcoefln = require( './../lib/matrix.js' ),
...
describe( 'matrix binomcoefln', function tests() {
	...
	it( 'should throw an error if provided unequal length matrices', function test() {
		...
		function badValues() {
			binomcoefln( matrix( [10,10] ), mat, 1 );
		}
	});
	...
	it( 'should throw an error if provided a y matrix which is not of equal dimensionality as the x matrix', function test() {
		expect( badValues ).to.throw( Error );
		function badValues() {
			binomcoefln( matrix( [5,5] ), mat, matrix( [10,10] ) );
		}
	});
	...
	it( 'should throw an error if provided an array as the y argument', function test() {
		...
		function foo() {
			binomcoefln( matrix( [5,5] ), mat, [1,2,3,4,5] );
		}
	});
	...
	it( 'should evaluate the binomcoefln function for a matrix and a scalar', function test() {
		...
		actual = binomcoefln( actual, mat, 2 );

		assert.isTrue( deepCloseTo( actual.data, out2.data, 1e-7 ) );
	});

	it( 'should evaluate the binomcoefln function for a matrix and a matrix', function test() {
		var actual;

		actual = matrix( [5,5], 'float64' );
		actual = binomcoefln( actual, mat, mat );
	...
	it( 'should return an empty matrix if provided an empty matrix', function test() {
		...
		mat = matrix( [0,10] );
		assert.deepEqual( binomcoefln( out, mat, 1 ).data, expected );
