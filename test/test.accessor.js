...
var // Expectation library:
	...
	// Module to be tested:
	binomcoefln = require( './../lib/accessor.js' );
...
describe( 'accessor binomcoefln', function tests() {

	it( 'should export a function', function test() {
		expect( binomcoefln ).to.be.a( 'function' );
	});

	it( 'should evaluate the binomcoefln function using an accessor when y is a scalar', function test() {
	...
	it( 'should evaluate the binomcoefln function using an accessor when y is an array', function test() {
	...
	it( 'should return empty array if provided an empty array', function test() {
		assert.deepEqual( binomcoefln( [], [], 1, getValue ), [] );
		function getValue( d ) {
			return d.x;
		}
	});
	...
	it( 'should throw an error if provided a y array which is not of equal length to the x array', function test() {
		...
		function foo() {
			binomcoefln( [], [1,2], [1,2,3], getValue );
		}
		function getValue( d ) {
			return d;
		}
	...
	it( 'should throw an error if provided a typed array as y which is not of equal length to the x array', function test() {
		expect( foo ).to.throw( Error );
		function foo() {
			binomcoefln( [], [1,2], new Int32Array( [1,2,3] ), getValue );
		}
		function getValue( d ) {
			return d;
		}
	...
	it( 'should throw an error if provided a matrix as y argument', function test() {
		expect( foo ).to.throw( Error );
		function foo() {
			binomcoefln( [], [1,2,3,4], matrix( new Int32Array( [1,2,3,4] ), [2,2] ), getValue );
		}
		function getValue( d ) {
			return d;
		}
